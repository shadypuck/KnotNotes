...
\usetikzlibrary{knots,decorations.markings,positioning,shapes.geometric,calc,backgrounds,fadings}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item The trefoil knot in Figure \ref{fig:circletrefoilb} is a three-crossing knot because it crosses itself 3 times.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item This definition is analogous to composite integers, where an integer is composite if it is the product of positive integers, neither of which is $1$.
    ...
    \begin{itemize}
        \item The unknot is not composite in the same way that 1 is not the product of two integers greater than 1.
    \end{itemize}
    \begin{figure}[h!]
        \centering
    ...
    \item \emph{Exercise 1.8}: Using the appendix table, identify the factor knots that make up the composite knot in Figure \ref{fig:ex1-8}.
    \begin{figure}[H]
        \centering
    ...
    \end{figure}
    \begin{itemize}
        \item The knot in Figure \ref{fig:ex1-8} is the composition of two projections of $5_2$.
    \end{itemize}
    \item \emph{Exercise 1.9}: Show that the knot in Figure \ref{fig:ex1-9a} is composite.
    \begin{figure}[h!]
        \centering
...
\begin{itemize}
    ...
    \item \textbf{Planar isotropy}: A deformation of \dq{the projection plane as if it were made of rubber with the projection drawn upon it}{12}
    \begin{itemize}
    ...
    \begin{figure}[h!]
        ...
        \label{fig:reidem}
    \end{figure}
    \item \textbf{Reidemeister move}: \dq{One of three ways to change a projection of the knot that \underline{will} change the relation between the crossings}{13}
    ...
    \item \textbf{Amphicheiral}: A knot that \dq{is equivalent to its mirror image, that is, the knot obtained by changing every crossing\dots to the opposite crossing}{14-15} \emph{Also known (by chemists) as} \textbf{achiral}.
    \begin{itemize}
...
\begin{itemize}
        ...
        \begin{subfigure}[b]{0.3\linewidth}
            ...
            \begin{tikzpicture}
                \begin{knot}[
                    ...
                    ignore endpoint intersections=false,
                    consider self intersections=true,
                    every strand/.append style={black,very thick}
                ]
                    \strand circle (1cm);
                    \strand plot[smooth cycle,tension=1.2] coordinates{
                        (-0.9,0.3) (0.9,-0.3) (0.9,0.3) (-0.9,-0.3)
                    };
                    \flipcrossings{3,5}
                \end{knot}
    ...
    \begin{itemize}
        \item The Whitehead link (Figure \ref{fig:WhiteheadBorromeana}) is a link of 2 components.
        \item The Borromean rings (Figure \ref{fig:WhiteheadBorromeanb}) are a link of 3 components.
        \item A knot is a link of 1 component.
    ...
    \begin{itemize}
        \item Both the linking number and the number of components are link invariants.
    \end{itemize}
...
\begin{itemize}
    \item How do we \underline{prove} that every knot is not just a projection of the unknot?
    ...
    \begin{itemize}
        ...
        \item A Type II Reidemeister move generates two crossings. If the strands are the same color, then everything stays the same color. If the strands are different colors, than newly created strand takes on the third color.
        \item Type III has many cases$^[$\footnote{These are available as an exercise in \emph{Exercise 1.23}.}$^]$.
    \end{itemize}
...
\begin{itemize}
    ...
    \end{figure}
    \item \textbf{Stick number}: \dq{The least number of straight sticks necessary to make a knot $K$}{29} \emph{Also known as} \textbf{\emph{s}(\emph{K})}.
...
\begin{itemize}
    ...
    \item Early on, there were few attempts to \underline{prove} that the tabulated knots were actually distinct.
    ...
    \begin{itemize}
        \item Note that the number of knots of successive numbers of crossings \underline{appears} to grows exponentially (this is unproven).
...
\begin{itemize}
    ...
    \begin{figure}[h!]
        ...
        \begin{subfigure}[b]{0.2\linewidth}
            \centering
            \includegraphics[width=0.7\linewidth]{Blender/ex2-3a.png}
            \caption{$6_2$.}
            \label{fig:ex2-3a}
        \end{subfigure}
        \begin{subfigure}[b]{0.2\linewidth}
    ...
    \begin{itemize}
        ...
        \item If the knot is amphicheiral, then the sequence only represents one knot.
    \end{itemize}
    \item Two Dowker reconstructions may be equivalent when projected onto a sphere as opposed to a plane.
...
\begin{itemize}
        ...
        \begin{subfigure}[b]{0.3\linewidth}
            ...
            \begin{tikzpicture}
                \begin{knot}[
                    ...
                    \strand (1,0)
                        to[out=30,in=-90]              (2,1)
                        to[out=90,in=-90]              (1.5,2)
                        to[out=90,in=-90]              (2.5,3.3)
                        to[out=90,in=90,looseness=1.5] (1,3.3)
                        to[out=-90,in=90]              (2,2)
                        to[out=-90,in=90]              (1.5,1)
                        to[out=-90,in=180]             (2.5,0)
                        to[out=0,in=180]               (4,2.5)
                        to[out=0,in=120]               (5,1.5)
                    ;
                    \strand (0,3.6)
                        to[out=-45,in=180] (2.7,3.5)
                        to[out=0,in=180]   (4,1.8)
                        to[out=0,in=-135]  (5,2.6)
                    ;
                    \flipcrossings{2,5,7}
                \end{knot}
                \fill [white,scale=0.5] (5,2.6) rectangle (3.5,3);
                \fill [white,scale=0.5] (5,2.6) -- (4.5,2.4) -- (4.1,2.7) -- cycle;
            \end{tikzpicture}
        ...
        \begin{subfigure}[b]{0.3\linewidth}
            ...
            \begin{tikzpicture}
                \begin{knot}[
                    ...
                    \strand (1,0)
                        to[out=10,in=-90] (2.5,1)
                        to[out=90,in=-90] (1,3)
                        to[out=90,in=180] (1.7,3.8)
                        to[out=0,in=180]  (5,1.5)
                        to[out=0,in=180]  (6,2)
                        to[out=0,in=120]  (7,0.8)
                    ;
                    \strand (0,3.8)
                        to[out=-45,in=180,out looseness=2] (2.9,3.8)
                        to[out=0,in=90]    (3.5,3)
                        to[out=-90,in=90]  (1.8,1)
                        to[out=-90,in=180] (2.5,0.2)
                        to[out=0,in=180]   (5,2)
                        to[out=0,in=180]   (6,1.5)
                        to[out=0,in=-90]   (7,2.9)
                    ;
    ...
    \end{itemize}
    \item \textbf{Pretzel knot}: \dq{A knot obtained from a tangle represented by a finite number of integers separated by commas}{48} See Figure \ref{fig:-357pretzel} for an example.
    ...
    \begin{itemize}
        ...
        \item The trefoil knot (Figure \ref{fig:circletrefoilb}) is an algebraic link of the 3 tangle (Figure \ref{fig:tanglesc}).
    \end{itemize}
    ...
    \item \textbf{Mutant} (knots): Two knots that are related by one or more mutations.
...
\begin{itemize}
    \item \textbf{Unknotting number}: A value $n\in\mathbb{N}$ specific to a knot $K$ that gives the fewest number of crossing changes needed in any projection to turn it into the unknot. \emph{Also known as} \textbf{\emph{u}(\emph{K})}.
    \begin{itemize}
        \item It may, however, be a nasty projection of the unknot (see Figure \ref{fig:ex1-11} and the related \emph{Exercise 1.11*}).
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item $K$ is distinct from the unknot, so $u(K)\neq 0$. The next possibility ($u(K)=1$) succeeds with the projection in Figure \ref{fig:figure8knot} (in fact, flipping \underline{any} crossing in the referenced projection trivializes $K$).
    \end{itemize}
    \item Note that performing all of the crossing changes on one projection is equivalent to performing one such change, then an ambient isotropy, then another.
    \begin{itemize}
    ...
    \begin{itemize}
        ...
        \item When you arrive at the first crossing, change it so that the strand that you are on is the overstrand, if necessary.
    ...
    \begin{itemize}
        \item If $k$ is positive, then the overstrand has a positive slope. If negative, vice versa.
    \end{itemize}
    \begin{figure}[h!]
        \centering
        \begin{subfigure}[b]{0.2\linewidth}
        ...
        \label{fig:k-moves}
    \end{figure}
    \item \textbf{\emph{k}-equivalent}: Two knot or link projections that can be transformed into each other through a series of $k$-moves.
    \begin{itemize}
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Bridge number}: The least number of unknotted arcs lying above the plane in any projection of the knot. \emph{Also known as} \textbf{\emph{b}(\emph{K})}.
    \begin{itemize}
    ...
    \begin{itemize}
        \item If a knot has bridge number 1, there exists a projection of the knot such that there is only one arc above the projection plane and one arc below the projection plane.
    ...
    \begin{itemize}
        ...
        \item Therefore, the next possibility is 2. If there is a projection of $5_2$ with bridge number 2, $b(K)=2$.
        \begin{figure}[h!]
            \centering
            %Redo in tikz?
            \begin{subfigure}[b]{0.2\linewidth}
...
\begin{itemize}
    \item \textbf{Crossing number}: \dq{The least number of crossings that occur in any projection of the knot}{67} \emph{Also known as} \textbf{\emph{c}(\emph{K})}.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Think of $D$ as a triangle. During the compression, three vertices and three edges separate into two \underline{pairs} of three vertices and three edges. This does not change the Euler characteristic.
    ...
    \item \textbf{Swallow-follow torus}: A torus that encompases (swallows) one factor knot and encases the strand of (follows; as if the strand has been thickened) the other factor knot.
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Therefore, the surface without boundary corresponding to Figure \ref{fig:boundsurfidenta} is a sphere.
    \end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item When assigning an orientation to this crossing, the directions on the subarcs \underline{must} be reversed.
        \begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Every twist knot can be projected in the mold of Figure \ref{fig:twistknots}, where the dots are replaced by some number of half-twists.
        \item For the trefoil knot, this number of twists is 0 and $s\overset{\checkmark}{=}c-1$.
        \item Each new half-twist obviously adds one crossing.
    ...
    \end{itemize}
    \item Returning to Section \ref{sss:Composition}, the trivial knot cannot be the composition of two nontrivial knots because $g(K)\geq 1$ where $K$ is nontrivial, so by Equation \ref{eqn:genuscomp}, $g(J\#K)\geq 2>0\Rightarrow g(J\#K)$ is nontrivial.
    ...
    \begin{itemize}
        \item For $c(K)=n$, the Dowker notation will consist of $n$ even numbers that are elements of $2\mathbb{N}\cap[2,2n]$. Therefore, there are $n$ possibilities for the first number, $n-1$ for the second number, and so on and so forth. This compresses down to $n!$ possible knots of $c(K)=n$.
        \begin{itemize}
            \item Note that a large number of these knots will not be of this crossing number. This is an upper bound in an extreme sense.
        \end{itemize}
        \item \textsc{Solution 1}: Use Equation \ref{eqn:genuscomp} to prove that the genus must increase with successive compositions. Since the genus is directly proportional to the quantity $c-s$ (by Equation \ref{eqn:gensei}), $c-s$ must increase with successive compositions. Since $s$ cannot decrease with successive compositions (and certainly cannot decrease infinitely), $c$ must increase with successive compositions.
        \item \textsc{Solution 2}: Composite knots are distinct from their factor knots. Generating a new composite knot creates a new knot. Since there are a finite number of knots of $c(K)=n$, eventually, all low-crossing possibilities (which will not all be reached, but regardless) must be used up. This forces $c(K)$ higher and higher as new knots are created.
    \end{itemize}
    ...
    \item \textbf{Canonical genus} (of a knot): \dq{The minimal genus of any Seifert surface obtained by applying Seifert's algorithm to a projection of the knot}{106} \emph{Also known as} \textbf{\emph{g\textsubscript{c}}(\emph{K})}.
...
\begin{itemize}
    ...
    \item This was proven in 1923 by J. W. Alexander of the Alexander polynomial, which is discussed in Section \ref{sse:polynomials}.
    ...
    \end{itemize}
    \item \textbf{Braid index}: \dq{The least number of strings in a braid corresponding to a closed braid representation of the link}{132} \emph{Also known as} \textbf{\emph{b}(\emph{L})}.
    \begin{table}[h!]
        \centering
        \renewcommand{\arraystretch}{1.4}
...
\begin{itemize}
    ...
    \end{align*}
    \begin{itemize}
        \item Based on the above, the bracket polynomial \underline{is} changed by a Type I Reidemeister move.
    ...
    \item \textbf{Writhe}: The sum of the $+1$s and $-1$s at every crossing. \emph{Also known as} \textbf{\emph{w}(\emph{L})}.
    \begin{itemize}
...
\begin{itemize}
    ...
    \item \emph{Lemma}: If $K$ has a reduced alternating projection of $n$ crossings, then $\text{span}(\lbq K\rbq)=4n$.
    \begin{itemize}
    ...
    \end{itemize}
    \item \emph{Theorem}: \dq{Two reduced alternating projections of the same knot have the same number of crossings}{162}
    \begin{itemize}
    ...
    \item \emph{Corollary}: $c(K_1\#K_2)=c(K_1)+c(K_2)$ for all $K_1\#K_2$ alternating knots.
    \begin{itemize}
...
\subsection{The Alexander and HOMFLY Polynomials}\label{sss:Alexander}
\begin{itemize}
    ...
    \item \emph{Exercise 6.15}: Show that the Alexander polynomial of a splittable link is always 0. Hint: Picture the splittable link as $L_0$ from Figure \ref{fig:ex6-8}.
    \begin{itemize}
        \item A splittable link will have a projection where the two components have no crossings.
        \item Because of this, place one link component adjacent to the other but not touching. This will be $L_0$.
        \item When the skein relation from Figure \ref{fig:ex6-8} is employed, $L_+$ and $L_-$ become the same composite knot --- one of the two components can be rotated 180$^\circ$ about the horizontal axis to yield the projection of the other$^[$\footnote{Figure \ref{fig:HOMFLYtrivial2comp} may help to visualize this.}$^]$.
        \item Because $L_+=L_-$, $\Delta(L_+)=\Delta(L_-)$.
        \item Therefore, Rule 2 can be used to yield the following, which simplifies to prove that the Alexander polynomial of a split(able) link ($\Delta(L_0)$) is equal to zero.
        \begin{align*}
            \Delta(L_+)-\Delta(L_+)+\left( t^{\frac{1}{2}}-t^{-\frac{1}{2}} \right)\Delta(L_0) &= 0\\
            \left( t^{\frac{1}{2}}-t^{-\frac{1}{2}} \right)\Delta(L_0) &= 0\\
            \Delta(L_0) &= 0
        \end{align*}
    \end{itemize}
    ...
    \item The HOMFLY polynomial \underline{can} always be computed, but the computations can take a long time.
    \begin{itemize}
    ...
    \end{itemize}
    \item \textbf{Complete invariant}: A characteristic that distinguishes all entities in a set.
...
\begin{itemize}
    ...
    \item Indeed, in 1988, they announced the first successful synthesis of a knotted molecule$^[$\footnote{As revealed by the epilogue (Knot Jokes and Pastimes), the chemists synthesized a M\"{o}bius band with three half-twists and then cut it open along its center line.}$^]$.
...
\begin{itemize}
    ...
    \end{itemize}
    \item That being said, if a molecule is topologically chiral, it \underline{must} be chemically chiral.
...
\begin{itemize}
    ...
    \item John Conway and Cameron Gordon (see Section \ref{sss:poincare} for their other exploits) published "Knots and Links in Spatial Graphs" in 1983, which proved the following theorem.
    \item \emph{Theorem}: Every embedding of $\mathcal{K}_6$ contains at least one pair of linked triangles.
    \begin{figure}[h!]
        \centering
        \begin{tikzpicture}[
            node distance=1cm,
    ...
    \item \emph{Exercise 8.5}: $\mathcal{K}_{3,3,1}$ is a Petersen graph. It can be obtained from $\mathcal{K}_6$ by triangle-$\gamma$ exchanges.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Arf invariant}: Let the value be 0 if the knot is pass equivalent to the unknot and 1 if the knot is pass equivalent to the trefoil knot. \emph{Also known as} \textbf{\emph{a}(\emph{K})}.
    ...
    \item Note that $\mathcal{K}_{5,5}$ is a \textbf{complete bipartite} graph.
    \item \textbf{Complete bipartite} (graph): A bipartite graph in which every vertex in one set is attatched with an edge to every vertex in the other set.
...
\begin{itemize}
    ...
    \begin{figure}[h!]
        \centering
        \begin{tikzpicture}
            \colorlet{ylx}{yellow!80!black}

            \foreach \r in {1,0.3} {
                \draw [ylx,thick] circle (\r);
                \draw [ylx,thick] (-\r,0) arc[start angle=-180,end angle=0,x radius=\r,y radius=0.5*\r];
                \draw [ylx,thick,dashed,dash pattern=on 3pt off 1pt] (-\r,0) arc[start angle=180,end angle=0,x radius=\r,y radius=0.5*\r];
            }

            \foreach \r in {0,15,35,60,90,120,145,165,180} {
                \draw [ylx,thick] (\r:0.3) -- (\r:{(1+3*sin(\r)^2)^(-0.5)});
            }
            \foreach \r in {195,215,240,270,300,325,345} {
                \draw [ylx,thick] (\r:{0.3*(1+3*sin(\r)^2)^(-0.5)}) -- (\r:{(1+3*sin(\r)^2)^(-0.5)});
            }
        \end{tikzpicture}
    ...
    \begin{figure}[h!]
        ...
        \begin{subfigure}[b]{0.2\linewidth}
            ...
            \begin{tikzpicture}
                ...
                \begin{scope}[yshift=1cm]
                    \foreach \r in {1,0.3} {
                        \draw [ylx,thick] (-\r,0) arc[start angle=180,end angle=0,radius=\r];
                        \draw [ylx,thick] (-\r,0) arc[start angle=-180,end angle=0,x radius=\r,y radius=0.5*\r];
                        \draw [ylx,thick,dashed,dash pattern=on 3pt off 1pt] (-\r,0) arc[start angle=180,end angle=0,x radius=\r,y radius=0.5*\r];
                    }
                ...
                \begin{scope}[yshift=-1cm]
                    \foreach \r in {1,0.3} {
                        \draw [ylx,thick] (-\r,0) arc[start angle=-180,end angle=0,radius=\r];
                        \draw [ylx,thick] ellipse ({\r} and {0.5*\r});
                    }
        
                    \foreach \r in {0,15,35,60,90,120,145,165,180,195,215,240,270,300,325,345} {
                        \draw [ylx,thick] (\r:{0.3*(1+3*sin(\r)^2)^(-0.5)}) -- (\r:{(1+3*sin(\r)^2)^(-0.5)});
                    }
                \end{scope}
            \end{tikzpicture}
        ...
        \begin{subfigure}[b]{0.2\linewidth}
            ...
            \begin{tikzpicture}
                ...
                \begin{scope}[yshift=1cm]
                    \draw [ylx,thick] (0,1) ellipse (0.65 and 0.325);
                    \draw [ylx,thick] (-0.65,0) arc[start angle=-180,end angle=0,x radius=0.65,y radius=0.325];
                    \draw [ylx,thick,dashed,dash pattern=on 3pt off 1pt] (-0.65,0) arc[start angle=180,end angle=0,x radius=0.65,y radius=0.325];
                    \foreach \r in {15,35,60,90,120,145,165} {
                        \draw [ylx,thick,dashed,dash pattern=on 3pt off 1pt] (\r:{0.65*(1+3*sin(\r)^2)^(-0.5)}) -- ([yshift=1cm]\r:{0.65*(1+3*sin(\r)^2)^(-0.5)});
                    }
                    \foreach \r in {0,180,195,215,240,270,300,325,345} {
                        \draw [ylx,thick] (\r:{0.65*(1+3*sin(\r)^2)^(-0.5)}) -- ([yshift=1cm]\r:{0.65*(1+3*sin(\r)^2)^(-0.5)});
                    }
                \end{scope}
                \begin{scope}[yshift=-1cm]
                    \draw [ylx,thick] (0,1) ellipse (0.65 and 0.325);
        ...
        \begin{subfigure}[b]{0.2\linewidth}
            ...
            \begin{tikzpicture}
                ...
                \begin{scope}[yshift=1cm]
                    \draw [ylx,thick] ellipse (1.5 and 0.9);
                    \draw [ylx,thick] (-0.2,0) arc[start angle=-180,end angle=0,x radius=0.2,y radius=0.05];
                    \draw [ylx,thick] (-0.15,-0.03) arc[start angle=180,end angle=0,x radius=0.15,y radius=0.03];
                \end{scope}
                \begin{scope}[yshift=-1cm]
                    \draw [ylx,thick] ellipse (1.5 and 0.9);
                    \draw [ylx,thick] (-0.2,0) arc[start angle=-180,end angle=0,x radius=0.2,y radius=0.05];
...
\subsection{The Poincar\'{e} Conjecture, Dehn Surgery, and the Gordon-Luecke Theorem}\label{sss:poincare}
\begin{itemize}
    ...
    \begin{figure}[h!]
        ...
        \begin{tikzpicture}
            \begin{knot}[
                ...
                \strand (1,-2) ellipse (0.7 and 0.3);
                \strand (-1.2,2)
                    to[out=90,in=90,looseness=1.8] (1.2,2)
                    to[out=-90,in=90] (0.8,0)
                    to[out=-90,in=90] (1.2,-2)
                    to[out=-90,in=-90,looseness=1.8] (-1.2,-2)
                    to cycle
                ;
                \strand (-0.8,2)
                    to[out=90,in=90,looseness=1.8] (0.8,2)
                    to[out=-90,in=-90,looseness=1.4] (1.8,1.8)
                    to[out=90,in=90,looseness=1.4] (0.8,1.6)
                    to[out=-90,in=90] (1.2,0)
                    to[out=-90,in=90] (0.8,-2)
                    to[out=-90,in=-90,looseness=1.8] (-0.8,-2)
                    to cycle
                ;
                \flipcrossings{1,2,6,8,9}
            \end{knot}
            \begin{knot}[
                clip width=5,
                clip radius=3pt,
                ...
                \strand (-0.8,2)
                    ...
                    to cycle
                ;
                \flipcrossings{1,2,6,7}
            \end{knot}
        \end{tikzpicture}
        \caption{Two distinct links with homeomorphic exteriors.}
        \label{fig:homeoexterior}
    \end{figure}
    \item There exist differing links with the same exterior (see Figure \ref{fig:homeoexterior}). These link exteriors are homeomorphic.
    \item \textbf{Property \emph{P}}: A property satisfied by a knot that has no surgery that could possibly yield a counterexample to the Poincar\'{e} conjecture.
    \item Property $P$ holds for the rational knots.
    \item Robion Kirby proved in 1978 the existence of the \textbf{Kirby calculus}.
    \item \textbf{Kirby calculus}: \dq{A set of simple operations that one can do to a Dehn surgery description of a three-manifold, such that if two different Dehn surgery descriptions yield the same three-manifold, they must be related through a sequence of these operations}{263}
    \begin{itemize}
...
\begin{itemize}
    ...
    \begin{figure}[h!]
        ...
        \begin{tikzpicture}
            \draw [help lines,dashed]
                ...
                (1,0.4) -- (1.3,0.7)
            ;
            \node [circle,fill,inner sep=1.5pt,label={right:$(x,y,z)$}] at (1.3,0.7) {};
    ...
    \begin{figure}[h!]
        ...
        \begin{subfigure}[b]{0.3\linewidth}
            \centering
            \begin{tikzpicture}
                \draw [->] (-1.4,0) -- (1.4,0) node[right]{$t$} node[below left]{Time};
                \draw [->] (0,-1.4) -- (0,1.4) node[above]{$z$};
                \draw [->] (1,1) -- (-1,-1) node[below left]{$x$};
            \end{tikzpicture}
            \caption{Axes.}
        ...
        \begin{subfigure}[b]{0.3\linewidth}
            \centering
            \begin{tikzpicture}
                \foreach \x in {0,0.15,...,0.75} {
                    \filldraw [thick,fill=white] (\x,0) -- (\x,1.5) -- ({\x+0.8},2.3) -- ({\x+0.8},0.8) -- cycle;
                }
                \draw [thick,-stealth] (1.5,0) node[above right]{Time} -- (2.8,0);
            \end{tikzpicture}
    ...
    \begin{figure}[h!]
        ...
        \begin{tikzpicture}[scale=0.7]
            \fill (-8,-1.6) rectangle (8,1.6);
            \foreach \x in {-8,-6,...,8} {
                \fill [white,xshift=\x cm] (-0.8,-0.8) rectangle (0.8,0.8);
            }
            \foreach \s in {-,} {
                \foreach \x in {-7.75,-7.25,...,7.75} {
                    \fill [white,xshift=\x cm,yshift=\s1.35cm] (-0.1,-0.15) rectangle (0.1,0.15);
                }
            }

            \begin{scope}[xshift=-4cm]
                \fill [blue] circle (3pt);
            \end{scope}
            \begin{scope}[xshift=-2cm]
                \draw [blue,thick] circle (12pt);
            \end{scope}
            \begin{scope}[xshift=0cm]
                \draw [blue,thick] circle (15pt);
            \end{scope}
            \begin{scope}[xshift=2cm]
                \draw [blue,thick] circle (12pt);
            \end{scope}
            ...
            \end{scope}
        \end{tikzpicture}
        \caption{A two-sphere in three-space, when one dimension is time.}
        \label{fig:movie-sphere3}
    \end{figure}
    \item For example, using such a coordinate system, a sphere centered at the origin will appear as a series of frames (like a movie; see Figure \ref{fig:movie-sphere3}) of a dot growing larger and then shrinking.
    \item Figure \ref{fig:movie-sphere3} describes a sphere in three-space using only two spatial dimensions.
    \item To describe four dimensions, use three spatial dimensions ($x$, $y$, and $z$) and one temporal dimension ($t$).
    \item Consider an unknotted sphere in four-space.
    \begin{figure}[h!]
        \centering
        \begin{tikzpicture}[scale=0.7]
            \fill (-6,-1.6) rectangle (6,1.6);
            \foreach \x in {-6,-4,...,6} {
                \fill [white,xshift=\x cm] (-0.8,-0.8) rectangle (0.8,0.8);
            }
            \foreach \s in {-,} {
                \foreach \x in {-5.75,-5.25,...,5.75} {
                    \fill [white,xshift=\x cm,yshift=\s1.35cm] (-0.1,-0.15) rectangle (0.1,0.15);
                }
            }

            \foreach \x in {-4,-2,...,4} {
                \draw [->,xshift=\x cm] (-0.7,-0.3) -- (0.1,-0.3);
                \draw [->,xshift=\x cm] (-0.3,-0.7) -- (-0.3,0.1);
                \draw [->,xshift=\x cm] (0,0) -- (-0.6,-0.6);
            }
            \begin{scope}[xshift=-4cm]
                \fill (0.4,0.1) [blue] circle (3pt);
            \end{scope}
            \begin{scope}[xshift=-2cm]
                \draw (0.4,0.1) [blue,thick] circle (6pt);
            \end{scope}
            \begin{scope}[xshift=0cm]
                \draw (0.4,0.1) [blue,thick] circle (8pt);
            \end{scope}
            \begin{scope}[xshift=2cm]
                \draw (0.4,0.1) [blue,thick] circle (6pt);
            \end{scope}
            ...
            \end{scope}
        \end{tikzpicture}
        \caption{Movie of a two-sphere in four-space.}
    ...
    \begin{figure}[h!]
        ...
        \begin{subfigure}[b]{0.19\linewidth}
            ...
            \begin{tikzpicture}
                \begin{knot}[
                    ...
                    \strand[red,line width=1mm] (90:1)
                        \foreach \x in {1,2,3} {
                            to [bend left=117,looseness=1.9] ({90+120*\x}:1)
                        }
                    ;
            ...
            \end{tikzpicture}
            \vspace{-0.8cm}
            \caption{$3_1$ in red slice.}
            \label{fig:trefoil-colorizeda}
        \end{subfigure}
        \begin{subfigure}[b]{0.19\linewidth}
            \centering
            \begin{tikzpicture}
                \begin{knot}[
                    ...
                    \flipcrossings{1,3}
                \end{knot}
                \draw [path fading=colorized,fit fading=false,fading transform={xshift=5mm,yshift=3mm,scale=0.8},yellow,line width=1mm] (210:1) to[bend left=117,looseness=1.9] (330:1);
            \end{tikzpicture}
            \vspace{-0.8cm}
            \caption{$3_1$ in mixed slices.}
            \label{fig:trefoil-colorizedb}
        \end{subfigure}
        \begin{subfigure}[b]{0.19\linewidth}
            \centering
            \begin{tikzpicture}
                \draw [red,line width=1mm] (210:1) to[bend left=117,looseness=1.9] (330:1);
                \begin{knot}[
                    clip width=2.5,
                    consider self intersections
                ]
                    \strand [path fading=colorized,fit fading=false,fading transform={xshift=5mm,yshift=3mm,scale=0.8},yellow,line width=1mm] (210:1)to[bend left=117,looseness=1.9] (330:1);
                    \strand [red,line width=1mm] (330:1)to[bend left=117,looseness=1.9] (450:1);
                    \strand [red,line width=1mm] (90:1)to[bend left=117,looseness=1.9] (210:1);
                    \flipcrossings{1,2,3}
                \end{knot}
            \end{tikzpicture}
            \vspace{-0.8cm}
        ...
        \begin{subfigure}[b]{0.19\linewidth}
            \centering
            \begin{tikzpicture}
                \draw [red,line width=1mm] circle (1cm);
                \draw [path fading=colorized,fit fading=false,fading transform={xshift=7mm,yshift=7mm,scale=0.8},yellow,line width=1mm] circle (1cm);
            \end{tikzpicture}
            \caption{Mixed loop.}
    ...
    \end{figure}
    \begin{itemize}
        \item By passing certain parts of the knot through itself in specific areas (flipping crossings), the knot can be unknotted. Recall \emph{Exercise 1.7*} and Figure \ref{fig:knottotrivial}.
        \begin{itemize}
...
\subsection{Knotted Spheres in Four Dimensions}
\begin{itemize}
    \item The movie for an unknotted sphere in four-space is shown in Figure \ref{fig:movie-sphere4}. It consists of variously sized loops.
    \begin{figure}[h!]
        \centering
        \begin{tikzpicture}[scale=0.7]
            \fill (-6,-1.6) rectangle (6,1.6);
            ...
            \end{scope}
        \end{tikzpicture}
        \caption{Movie of a knotted two-sphere in four-space.}
        \label{fig:movie-trefoilsphere}
    \end{figure}
    \item These loops can be knotted. Such knotting, as in Figure \ref{fig:movie-trefoilsphere}, depicts a knotted sphere in four-space.
    \item While Figure \ref{fig:movie-trefoilsphere} is the simplest way to visualize a knotted two-sphere in four-space, it has problems, too.
    \item Because of the zero-dimensional end points, the manifold cannot be modeled with differentiable functions.
    \item The ideal manifold would be \textbf{smooth}.
    \item \textbf{Smooth} (manifold): A manifold described by functions, all of whose derivatives exist.
    \item If the first and last frames of the movie have two or more \textbf{saddle points}, the problem is solved.
    \item \textbf{Saddle point}: A point on the surface of a manifold where the slopes (derivatives) in all orthogonal directions are zero.
    \item Use the Euler characteristic to prove that the surface pictured in Figure \ref{fig:movie-trefoilsphere} is still a sphere.
    \begin{itemize}
        \item Let the trefoil knot in the middle frame be represented by a stick picture, as in Figure \ref{fig:sticktrefoil}.
        \item Let the points in the first and last frames be vertices.
        \item Because $s(3_1)=6$ and the first and last points count as vertices, $V=8$.
        \item Six edges are present in the stick picture. Twelve further edges connect each vertex in the stick picture to the vertex in the first and last frames. Thus, $E=18$.
        \item Counting up faces (note that some of them intersect in three-dimensions) reveals that $F=12$.
        \item Therefore, the following use of Equation \ref{eqn:eulerchar} proves that the manifold in Figure \ref{fig:movie-trefoilsphere} is a two-sphere.
        \begin{equation*}
            \chi = 8-18+12 = 2
        \end{equation*}
    \end{itemize}
    \item It can be proved that the knotted two sphere in Figure \ref{fig:movie-trefoilsphere} cannot be unknotted via a \textbf{rubber deformation}$^[$\footnote{A more casual term for an ambient isotropy. Used throughout the book, but generally not mentioned in favor of the formal term.}$^]$ in four-space.
\end{itemize}


\subsection{Knotted Three-Spheres in Five-Space}
\begin{itemize}
    \item Using both the color model and the temporal model at the same time, it is possible to picture five-space.
    \item Use three spatial dimensions ($x$, $y$, and $z$), one temporal dimension ($t$), and one color dimension $c$.
    \item Visualizing the three-sphere in five-space:
    \begin{figure}[h!]
        \centering
        \begin{tikzpicture}
            \colorlet{rex}{red}
            ...
            \colorlet{grx}{green!50!black}
            \colorlet{blx}{blue}
            \colorlet{inx}{red!20!blue!80}
            \colorlet{vlx}{violet}
            \def\r{0.2}
            \def\rr{0.33}
            \def\rrr{0.4}

            \draw [help lines] (0.1,0.1) grid (8,8);
            \foreach \y in {1,...,8} {
                \draw [help lines] (-0.5,\y) -- (0.1,\y);
            }
            \foreach \y in {1,...,7} {
                \node at (0.2,{\y+0.5}) {$t=\y$};
            }
            \foreach \x/\l in {
                1.5/R,
                2.5/O,
            ...
            \fill [rex] (1.5,4.5) circle (3pt);
            \fill [orx] (2.5,3.5) circle (3pt);
            \fill [ylx] (3.5,2.5) circle (3pt);
            \fill [grx] (4.5,1.5) circle (3pt);
            \fill [blx] (5.5,2.5) circle (3pt);
            \fill [inx] (6.5,3.5) circle (3pt);
            \fill [vlx] (7.5,4.5) circle (3pt);
            \fill [inx] (6.5,5.5) circle (3pt);
            \fill [blx] (5.5,6.5) circle (3pt);
            \fill [grx] (4.5,7.5) circle (3pt);
            \fill [ylx] (3.5,6.5) circle (3pt);
            \fill [orx] (2.5,5.5) circle (3pt);

            \draw [orx,thick] (2.5,4.5) circle (\r);
            \draw [ylx,thick] (3.5,3.5) circle (\r);
            \draw [grx,thick] (4.5,2.5) circle (\r);
            \draw [blx,thick] (5.5,3.5) circle (\r);
            \draw [inx,thick] (6.5,4.5) circle (\r);
            \draw [blx,thick] (5.5,5.5) circle (\r);
            \draw [grx,thick] (4.5,6.5) circle (\r);
            \draw [ylx,thick] (3.5,5.5) circle (\r);

            \draw [ylx,thick] (3.5,4.5) circle (\rr);
            \draw [grx,thick] (4.5,3.5) circle (\rr);
            \draw [blx,thick] (5.5,4.5) circle (\rr);
            \draw [grx,thick] (4.5,5.5) circle (\rr);

            \draw [grx,thick] (4.5,4.5) circle (\rrr);
        \end{tikzpicture}
        \caption{A three-sphere in five-space.}
        \label{fig:3-sphere5-space}
    \end{figure}
    \begin{itemize}
        \item According to \emph{Exercise 10.5}, the three-sphere in the temporal model of four-space looks like a growing and shrinking two-sphere.
        \item Now, the temporal slices of the three-sphere sit in \dq{four-dimensional slices of five-space}{273}
        \item Let each of these four-dimensional slices have multiple different colors, so that we can see the three-sphere in the remaining spatial dimensions, as in Figure \ref{fig:3-sphere5-space}$^[$\footnote{Note that coordinate axes are not shown in Figure \ref{fig:3-sphere5-space}. Rather, they are left to the imagination, so as to leave more room for the pictures. The axes will be inferred unless stated otherwise from here-on out.}$^]$.
        \item For any single color (or moment in time), the time (or color) slices compose a two-sphere in four-space.
    \end{itemize}
    \item When considering a knotted three-sphere, each colored slice (represented by a column of pictures) intersects the three-sphere in a point, unknotted two-sphere, or knotted two-sphere.
    \item We can use other attributes to be dimensions (such as brightness, noise, etc.).
    \begin{itemize}
        \item In this manner, it is possible to imagine any dimension.
    \end{itemize}
\end{itemize}
